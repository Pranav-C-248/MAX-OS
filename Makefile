...
kernel.bin: kernel-entry.o kernel.o util.o screen_driver.o interrupts.o idt.o isr.o shell.o keyboard.o 
	ld -m elf_i386 -o $@ -Ttext 0x1000 $^ --oformat binary

kernel-entry.o: kernel_entry.asm
	nasm $< -f elf -o $@

kernel.o: kernel.c screen_driver.h idt.h isr.h keyboard.h
	gcc -fno-pie -m32 -ffreestanding -c $< -o $@
...
clean:
	rm -f boot.bin shell.o kernel.bin final_raw_binary.bin kernel-entry.o kernel.o screen_driver.o idt.o isr.o keyboard.o interrupts.o util.o
